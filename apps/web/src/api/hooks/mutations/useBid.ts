...
export const useBidMutation = (
  ...
  return useMutation<BidResult, AxiosError, BidRequest, { previousData: unknown }>({
    ...
    onMutate: async (variables) => {
      ...
      const previousData = queryClient.getQueryData(queryKeys.auctions.detail(variables.auction_id))

      // Optimistically update the auction detail
      queryClient.setQueryData(queryKeys.auctions.detail(variables.auction_id), (old: any) => {
        if (!old) return old
        return {
          ...old,
